...
type HooksPostRequestBody struct {
    ...
    // The [events](https://docs.github.com/enterprise-server@3.14/webhooks/event-payloads) that trigger this webhook. A global webhook can be triggered by `user` and `organization` events. Default: `user` and `organization`.
    events []string
...
// returns a HooksPostRequestBody_configable when successful
func (m *HooksPostRequestBody) GetConfig()(HooksPostRequestBody_configable) {
    return m.config
}
// GetEvents gets the events property value. The [events](https://docs.github.com/enterprise-server@3.14/webhooks/event-payloads) that trigger this webhook. A global webhook can be triggered by `user` and `organization` events. Default: `user` and `organization`.
...
func (m *HooksPostRequestBody) GetFieldDeserializers()(map[string]func(i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode)(error)) {
    ...
    res["config"] = func (n i878a80d2330e89d26896388a3f487eef27b0a0e6c010c493bf80be1452208f91.ParseNode) error {
        val, err := n.GetObjectValue(CreateHooksPostRequestBody_configFromDiscriminatorValue)
        if err != nil {
            return err
        }
        if val != nil {
            m.SetConfig(val.(HooksPostRequestBody_configable))
        }
        return nil
    }
...
// SetEvents sets the events property value. The [events](https://docs.github.com/enterprise-server@3.14/webhooks/event-payloads) that trigger this webhook. A global webhook can be triggered by `user` and `organization` events. Default: `user` and `organization`.
...
type HooksPostRequestBodyable interface {
    ...
    GetActive()(*bool)
    GetConfig()(HooksPostRequestBody_configable)
    GetEvents()([]string)
    GetName()(*string)
    SetActive(value *bool)()
    SetConfig(value HooksPostRequestBody_configable)()
