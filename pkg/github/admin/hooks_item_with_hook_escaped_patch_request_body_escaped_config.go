...
type HooksItemWithHook_PatchRequestBody_config struct {
    ...
    insecure_ssl *string
    // If provided, the `secret` will be used as the `key` to generate the HMAC hex digest value in the [`X-Hub-Signature`](https://docs.github.com/enterprise-server@3.12/webhooks/event-payloads/#delivery-headers) header.
    secret *string
...
// GetSecret gets the secret property value. If provided, the `secret` will be used as the `key` to generate the HMAC hex digest value in the [`X-Hub-Signature`](https://docs.github.com/enterprise-server@3.12/webhooks/event-payloads/#delivery-headers) header.
// returns a *string when successful
