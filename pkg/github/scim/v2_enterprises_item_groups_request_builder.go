...
// V2EnterprisesItemGroupsRequestBuilderGetQueryParameters > [!NOTE]> The SCIM API endpoints for enterprise accounts are currently in private preview and are subject to change.Lists provisioned SCIM groups in an enterprise.You can improve query search time by using the `excludedAttributes` query parameter with a value of `members` to exclude members from the response.
...
// Get > [!NOTE]> The SCIM API endpoints for enterprise accounts are currently in private preview and are subject to change.Lists provisioned SCIM groups in an enterprise.You can improve query search time by using the `excludedAttributes` query parameter with a value of `members` to exclude members from the response.
...
// 
// [API method documentation]: https://docs.github.com/enterprise-server@3.11/rest/enterprise-admin/scim#list-provisioned-scim-groups-for-an-enterprise
...
// Post > [!NOTE]> The SCIM API endpoints for enterprise accounts are currently in private preview and are subject to change.Creates a SCIM group for an enterprise.If members are included as part of the group provisioning payload, they will be created as external group members. It is up to a provider to store a mapping between the `externalId` and `id` of each user.
...
// [API method documentation]
// 
// [API method documentation]: https://docs.github.com/enterprise-server@3.11/rest/enterprise-admin/scim#provision-a-scim-enterprise-group
...
// ToGetRequestInformation > [!NOTE]> The SCIM API endpoints for enterprise accounts are currently in private preview and are subject to change.Lists provisioned SCIM groups in an enterprise.You can improve query search time by using the `excludedAttributes` query parameter with a value of `members` to exclude members from the response.
...
// ToPostRequestInformation > [!NOTE]> The SCIM API endpoints for enterprise accounts are currently in private preview and are subject to change.Creates a SCIM group for an enterprise.If members are included as part of the group provisioning payload, they will be created as external group members. It is up to a provider to store a mapping between the `externalId` and `id` of each user.
// returns a *RequestInformation when successful
